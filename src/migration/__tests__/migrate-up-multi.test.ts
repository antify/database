...
describe('Migrate up multi connection test', async () => {
  ...
  test('Should migrate all tenants up to end', async () => {
    ...
    vi.spyOn(migrateUpFunctions, 'migrateUpToEnd').mockReturnValue(new Promise(async (resolve) => {
      resolve([
        {
    ...
    await migrateTenantsUpToEnd(client, '', callback);
  ...
  test('Should migrate all tenants up to a specific migration', async () => {
    ...
    vi.spyOn(migrateUpFunctions, 'migrateUpTo').mockReturnValue(new Promise(async (resolve) => {
      resolve([
        {
          executionTimeInMs: 100,
          migrationName: 'xyz',
    ...
    await migrateTenantsUpTo('xyz', client, '', callback);
  ...
  test('Should migrate all tenants one specific migration up', async () => {
    ...
    vi.spyOn(migrateUpFunctions, 'migrateOneUp').mockReturnValue(new Promise(async (resolve) => {
      resolve({
        executionTimeInMs: 100,
        migrationName: 'xyz',
      });
    ...
    await migrateTenantsOneUp('xyz', client, '', callback);
