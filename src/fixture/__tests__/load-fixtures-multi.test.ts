...
describe('Load fixtures multi connection test', async () => {
  const getMocks = () => {
    ...
    vi.spyOn(client, 'connect').mockReturnValue(new Promise((resolve) => resolve(client)));
  ...
  test('Should load all fixtures for all tenants', async () => {
    ...
    vi.spyOn(loadFixturesFunctions, 'loadFixtures').mockReturnValue(new Promise(async (resolve) => {
      resolve([
        {
    ...
    await loadFixturesMulticonnection(client, '', callback);
