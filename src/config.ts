...
export const defineDatabaseConfig = (config: DatabaseConfigurations): DatabaseConfigurations => {
  ...
  for (const databaseName of Object.keys(config)) {
    if (!config[databaseName].isSingleConnection) {
      if (
        ...
        throw new Error(`Configuration "${databaseName}" require "fetchTenants" is configured because it is a multi connection`);
      }
    }
  }
