...
import {
  MultiConnectionDropDatabaseCallbacks, dropDatabaseMulti,
} from '../drop-database-multi';
import {
...
describe('Drop database multi test', async () => {
  ...
  test('Should drop all databases of a multi connection', async () => {
    const databaseConfiguration: MultiConnectionDatabaseConfiguration = {
      ...
      fetchTenants: async () => {
        return new Promise((resolve) => {
          resolve([
            {
              id: '64c39fcbcb7afc335881326e',
              name: 'Foo tenant',
            },
            {
              id: '64c76796b3854de74305dab8',
              name: 'Bar tenant',
            },
          ]);
        });
    ...
    await dropDatabaseMulti(client, callbacks);
    ...
    expect((onDropDatabaseFinishedSpy.mock.calls[0][0]).tenantId).toBe('64c39fcbcb7afc335881326e');
    expect((onDropDatabaseFinishedSpy.mock.calls[0][0]).tenantName).toBe('Foo tenant');
    expect((onDropDatabaseFinishedSpy.mock.calls[0][0]).result.error).toBeNull();
    expect((onDropDatabaseFinishedSpy.mock.calls[0][0]).result.executionTimeInMs).toBeGreaterThan(0.5);

    expect((onDropDatabaseFinishedSpy.mock.calls[1][0]).tenantId).toBe('64c76796b3854de74305dab8');
    expect((onDropDatabaseFinishedSpy.mock.calls[1][0]).tenantName).toBe('Bar tenant');
    expect((onDropDatabaseFinishedSpy.mock.calls[1][0]).result.error).toBeNull();
    expect((onDropDatabaseFinishedSpy.mock.calls[1][0]).result.executionTimeInMs).toBeGreaterThan(0.5);

    expect(beforeDropDatabase.mock.calls).toStrictEqual([
      [
        '64c39fcbcb7afc335881326e',
        'Foo tenant',
      ],
      [
        '64c76796b3854de74305dab8',
        'Bar tenant',
      ],
    ]);
  });
  ...
  test('Should trigger event if no databases are there to delete', async () => {
    ...
    // @ts-ignore
    MultiConnectionClient.instance = null;
